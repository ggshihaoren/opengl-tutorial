...
int main() 
{
    ...
    // 流程：创建帧缓冲对象和渲染缓冲对象，渲染缓冲对象作为附件存储信息并附加
    // 接着创建并绑定所需的贴图，根据输入图片的信息为其设定通道和过滤方式等信息
    // 此时设定好着色器的视角，透视投影矩阵等信息，设定即将采样纹理的图片等信息，并绑定帧缓冲对象，表示渲染到帧缓冲对象上
    // 设定视角，将创建好的贴图作为颜色附件绑定到帧缓冲对象上，然后渲染到帧缓冲对象上
    // 最后解绑帧缓冲对象，表示渲染到默认帧缓冲对象上
    ...
    // 分配和初始化一个渲染缓冲对象(RBO)来存储深度信息,24 位精度来存储深度值
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, 512, 512);
    // 将刚刚配置好的渲染缓冲对象附加到帧缓冲对象上,指定附件为深度附件
    ...
    for (unsigned int i = 0; i < 6; i++) {
        ...
        // 已创建的纹理绑定到 FBO 的颜色附件点
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, envCubemap, 0);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        // 执行渲染操作，结果将写入附加的纹理
        renderCube();

        // envCubemap 最终包含一个完整的立方体贴图，可用于后续的环境映射或天空盒渲染。
    }
    ...
    for (unsigned int i = 0 ; i < 6; i++) {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16, 128, 128, 0, GL_RGB, GL_FLOAT, nullptr);
    }
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
    // 计划采样 prefilterMap 的 mipmap，所以需要确保将其缩小过滤器设置为 GL_LINEAR_MIPMAP_LINEAR 以启用三线性过滤
